...
CAgoraExternalCaptureDlg::CAgoraExternalCaptureDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(CAgoraExternalCaptureDlg::IDD, pParent)
{
...
void CAgoraExternalCaptureDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CAgoraExternalCaptureDlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_WM_NCHITTEST()
	ON_MESSAGE(WM_GOBACK, &CAgoraExternalCaptureDlg::OnBackPage)
	ON_MESSAGE(WM_GONEXT, &CAgoraExternalCaptureDlg::OnNextPage)
	ON_MESSAGE(WM_JOINCHANNEL, &CAgoraExternalCaptureDlg::OnJoinChannel)
	ON_MESSAGE(WM_LEAVECHANNEL, &CAgoraExternalCaptureDlg::OnLeaveChannel)
	
	ON_BN_CLICKED(IDC_BTNMIN, &CAgoraExternalCaptureDlg::OnBnClickedBtnmin)
...
BOOL CAgoraExternalCaptureDlg::PreTranslateMessage(MSG* pMsg)
{
...
BOOL CAgoraExternalCaptureDlg::OnInitDialog()
{
...
void CAgoraExternalCaptureDlg::InitCtrls()
{
	CRect ClientRect;
...
void CAgoraExternalCaptureDlg::InitChildDialog()
{
	CString str;
...
void CAgoraExternalCaptureDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
void CAgoraExternalCaptureDlg::OnPaint()
{
...
HCURSOR CAgoraExternalCaptureDlg::OnQueryDragIcon()
{
...
LRESULT CAgoraExternalCaptureDlg::OnNcHitTest(CPoint point)
{
...
void CAgoraExternalCaptureDlg::DrawClient(CDC *lpDC)
{
	CRect	rcText;
	CRect	rcClient;
	...
	lpDC->SetTextColor(RGB(0x91, 0x96, 0xA0));
	lpDC->SetBkColor(RGB(0xFF, 0xFF, 0xFF));
	CString sversion = CString("SDK Version: ") +  CAgoraObject::GetSDKVersion().GetBuffer();
	lpString = sversion.GetBuffer();
...
void CAgoraExternalCaptureDlg::OnBnClickedBtnmin()
{
	// TODO:  在此添加控件通知处理程序代码
...
LRESULT CAgoraExternalCaptureDlg::OnBackPage(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CAgoraExternalCaptureDlg::OnNextPage(WPARAM wParam, LPARAM lParam)
{
	m_lpCurDialog->ShowWindow(SW_HIDE);
...
LRESULT CAgoraExternalCaptureDlg::OnJoinChannel(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CAgoraExternalCaptureDlg::OnLeaveChannel(WPARAM wParam, LPARAM lParam)
{
	CAgoraObject	*lpAgoraObject = CAgoraObject::GetAgoraObject();
...
LRESULT CAgoraExternalCaptureDlg::OnLastmileQuality(WPARAM wParam, LPARAM lParam)
{
